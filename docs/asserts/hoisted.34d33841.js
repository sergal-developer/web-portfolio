class r{constructor(){}injectData(a,n){let t=a;for(const i in n){const e=i;t=t.replaceAll(`{${e}}`,n[i])}return t}injectInterateData(a,n){let t="";return n.forEach(i=>{let e=a;for(const s in i){const o=`{${s}}`;e=e.replace(o,i[s])}t+=e}),t}}class d{constructor(){}template=new r;data={title:"About",name:"Sergio Gallegos",email:"sergio.gallegos@live.com.mx",phone:"52+ 5516322613",birthday:"13 June 1988",nationality:"Mexican",subtitle:"Professional Profile",profileDescription:`<p>
        ...
        development.
        </p>`,sign:"/web-portfolio/asserts/img/signature-web.png",documentUrl:"/web-portfolio/asserts/files/cv.pdf",picture:"/web-portfolio/asserts/img/sergal-front.png"};html=`<h2>{title}</h2>
    ...
    `;getData(){return this.template.injectData(this.html,this.data)}}class m{constructor(){}template=new r;data={title:"Education",subtitle:"Just My Education",description:`<p>Sergio started working as a designer and programmer in 2008. During his first years he worked in projects
    ...
    <div class="full-divider"></div>`;getData(){return this.data.listEducation=this.template.injectInterateData(this.listTemplate,this.data.list),this.template.injectData(this.html,this.data)}}class c{constructor(){}template=new r;data={title:"Experience",subtitle:"14 Years Experience!",description:`<p>
  ...
  <li class="timeline-event">
  ...
  </li>`;listTemplateTech="<div>{name}</div>";getData(){return this.data.list.forEach(a=>{a.listTech=this.template.injectInterateData(this.listTemplateTech,a.tech)}),this.data.listExperience=this.template.injectInterateData(this.listTemplate,this.data.list),this.template.injectData(this.html,this.data)}}class p{constructor(){}template=new r;data={title:"Hello, I'm ",name:"Sergio Gallegos",position:"Frontend Developer / Backend Developer"};html=`<div class="hero">
    <div class="text">
      <div class="static">
        <h2>
          <span>{title}</span><br>
    ...
    `;getData(){return this.template.injectData(this.html,this.data)}}class g{constructor(){}template=new r;data={title:"Portfolio",subtitle:"Just My Work",description:`<p>Sergio started working as a designer and programmer in 2008. During his first years he worked in projects
      with technologies such as: JS, and ASP. In addition, he honed skills in graphic design, web development,
      and desktop application development with Microsoft Tecnologies. Sergio acknowledges the benefits of agile
      methodologies to ensure predictive delivery in software projects. He has experience working with Scrum and
      Kanban methods. Additionally, Sergio has a solid background in design patterns, OOP, and software
  ...
  <div class="full-divider"></div>`;getData(){return this.data.listPortfolio=this.template.injectInterateData(this.listTemplate,this.data.list),this.data.listPortfolio="<h1>Under Construction ...</h1>",this.template.injectData(this.html,this.data)}}class h{constructor(){}template=new r;data={title:"Skills & Education",subtitle1:"Just My Skills",description1:"<p>Since 2008 I have developed knowledge in design, and programming oriented to backend and frontend, besides that I have worked under Scrum and Kanban methodologies, managed code using git scm in github and gitlab below I show you what technologies I have worked throughout these wonderful years.</p>",subtitle2:"Just My Education",description2:`<p>I studied at the Universidad Autonoma de México, 
    ...
    `;listTempleteSkillsLevels=['<div class="lvl"><span>rookie</span></div>','<div class="lvl"><span>intermediate</span></div>','<div class="lvl"><span>advanced</span></div>','<div class="lvl"><span>master</span></div>'];listTemplateEducation=`
  <div class="education-details">
        <h5>{name}</h5>
        <h4>{carrer}</h4>
        <span>{years}</span>
  </div>`;listTemplateLanguage=`
    <div class="item-skill">
      <div class="name">{name} <b>{level}</b></div>
      <div class="progressbar">
        <div class="progressbar-indicator" style="width: {value}%"></div>
      </div>
    </div>`;getData(){const a=[],n=[],t=[],i=[];return this.data.skills.forEach(e=>{const s=Object.create(this.listTempleteSkillsLevels);for(let o=0;o<e.level+1;o++)s[o]=s[o].replace('class="lvl"','class="lvl fill"');s[e.level]=s[e.level].replace('class="lvl fill"','class="lvl current"'),e.levelHtml=s.join(""),e.type.toLowerCase()==="frontend"&&n.push(e),e.type.toLowerCase()==="design"&&a.push(e),e.type.toLowerCase()==="backend"&&t.push(e),e.type.toLowerCase()==="admin"&&i.push(e)}),this.data.listSkillsFrontend=this.template.injectInterateData(this.listTemplateSkills,n),this.data.listSkillsDesign=this.template.injectInterateData(this.listTemplateSkills,a),this.data.listSkillsBackend=this.template.injectInterateData(this.listTemplateSkills,t),this.data.listSkillsAdmin=this.template.injectInterateData(this.listTemplateSkills,i),this.data.listEducation=this.template.injectInterateData(this.listTemplateEducation,this.data.education),this.data.listSoftSkills=this.template.injectInterateData(this.listTemplateLanguage,this.data.softSkills),this.data.listHardSkills=this.template.injectInterateData(this.listTemplateLanguage,this.data.hardSkills),this.template.injectData(this.html,this.data)}}class u{frames;frameCollector=null;links;inNavigation=!1;currentPage=0;transitionDurationFrame=1e3;frameName=".page-frame-container";frameContainerName=".pfc-area";frameContainerWidth="width: calc(100%);";frameItemsName=".pfc-area > .item";linksName=".navbar .item.page";navbarName=".navbar";shapesName=".shapes";navBar=null;frameContainer=null;currentPageClass="home";shapesContainer=null;data={home:new p,about:new d,skills:new h,experience:new c,education:new m,portfolio:new g};constructor(){}init(){this.setupEvents()}setupEvents(){this.navBar=document.querySelector(this.navbarName),this.frameCollector=document.querySelector(this.frameName),this.frames=document.querySelectorAll(this.frameItemsName),this.links=document.querySelectorAll(this.linksName),this.frameContainer=document.querySelector(this.frameContainerName),this.frameContainerWidth=`width: calc(100% * ${this.frames.length});`,this.frameContainer?.setAttribute("style",`${this.frameContainerWidth}`),this.shapesContainer=document.querySelector(this.shapesName),this.links.forEach((t,i)=>{t.addEventListener("click",e=>{this.gotoPage(i,t),document.querySelector("#navbar-menu")?.classList.remove("show")})});const a=document.querySelector("#navbar-menu");document.querySelector("#navbar-menu-button")?.addEventListener("click",t=>{a?.classList.contains("show")?a?.classList.remove("show"):a?.classList.add("show")})}gotoPage(a,n){if(this.inNavigation||this.currentPage===a)return;this.inNavigation=!0,this.currentPage=a;const t=n.getAttribute("data-class");console.log("dataClass: ",t);const i=`-${100*a}%`,e=`transition-duration: ${this.transitionDurationFrame}ms;`,s=`margin-left: ${i};`;this.frameContainer?.setAttribute("style",`${this.frameContainerWidth}${e}${s}`),t&&(this.frameContainer?.classList.replace(this.currentPageClass,t),this.shapesContainer?.classList.replace(this.currentPageClass,t),this.currentPageClass=t),this.frameContainer?.classList.add("moveOn"),setTimeout(()=>{this.frameContainer?.classList.remove("moveOn"),this.inNavigation=!1},this.transitionDurationFrame),this.currentPage!==0?this.navBar?.classList.contains("collapsed")||this.navBar?.classList.add("collapsed"):this.navBar?.classList.remove("collapsed")}setupData(){document.querySelector("#home"),document.querySelector("#about"),document.querySelector("#skills"),document.querySelector("#experience"),document.querySelector("#portfolio")}}const v=new u;window.onload=l=>{v.init(),console.log("loaded!")};
